...
def google_search(state: State):
    ...
    print(f"Searching Google (via Tavily) for: {user_question}")
    # Engine param is ignored by Tavily wrapper but kept for logging
...
def reddit_search(state: State):
    ...
    return {"reddit_results": str(reddit_results)}
...
def analyze_reddit_posts(state: State):
    ...
    if not reddit_results or reddit_results == "[]":
...
def retrieve_reddit_posts(state: State):
    print("Getting reddit post content")
...
def run_chatbot():
    print("Multi-Source Research Agent (Powered by Groq & Tavily)")
